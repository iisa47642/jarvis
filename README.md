...
**Codex CLI** оборачивается тем же механизмом: если при `npm run setup` найден `codex`, Jarvis ставит хуки в `~/.codex/hooks.json` (метка `codex`) и шим `~/.jarvis/shims/codex` (один скрипт `agent-shim`, поведение выбирается по `basename "$0"`). Интерактивные сессии `codex` попадают в панель с бейджем `codex`: статус, тосты, голос, чат, ответ-в-сессию, `codex resume`. Модель и reasoning меняются через `/model`-пикер Codex (отдельного `/effort` у него нет). **Headless `codex exec` хуки НЕ шлёт** — такие запуски Jarvis не мониторит (по дизайну, как и `claude -p`). На свежей машине хуки Codex требуют доверия (`~/.codex/config.toml [hooks.state]`); шим Codex добавляет `--dangerously-bypass-hook-trust` — это **снимает проверку доверия для всех хуков Codex** при интерактивном запуске, имей в виду этот компромисс.
