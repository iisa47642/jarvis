//! Трейт движка TTS + реализация Silero (клиент к локальному сайдкару).
...
/// Собрать движок. Silero — единственный движок (клиент к сайдкару).
/// `silero_base` — URL сайдкара (`http://127.0.0.1:PORT`).
pub fn build_engine(silero_base: String) -> Box<dyn TtsEngine> {
    Box::new(SileroEngine::new(silero_base))
}
...
mod tests {
    ...
    fn silero_unreachable_fails_safe() {
        ...
        assert!(!e.available());
        assert!(e
            .synthesize("привет", &VoiceSel { speaker: "baya".into(), sample_rate: 24000, rate: "fast".into() })
            .is_err());
    }

    #[test]
    fn build_engine_is_silero() {
        assert_eq!(build_engine("http://127.0.0.1:1".to_string()).name(), "silero");
    }
}
